...
class LieGroup:
    ...
    def reshape(self, dims):
        data_reshaped = self.data.reshape(dims + (self.embedded_dim,))
    ...
    def requires_grad_(self, grad_bool):
        self.data.requires_grad_(grad_bool)
        return self
...
class SE3(LieGroup):
    ...
    def interpolate(self, G, lam=0.5):
        t,q = self.data.split([3,4], -1)
        Gt, Gq = G.data.split([3,4], -1)
        tnew = lam*t + (1-lam)*Gt
        qnew = self.quat_interpolate(q, Gq, lam)
        return SE3(torch.cat([tnew, qnew], dim=-1))

    def quat_interpolate(self, q1, q2, t):
        theta = torch.acos((q1*q2).sum(dim=-1))
        mask = theta > np.pi
        q2 = mask*(-q2) + (1-mask)*q2
        theta = mask*(2*np.pi - theta) + (1-mask)*theta
        mask1 = (theta == 0.0)
        sin_1_t = torch.sin((1-t)*theta) + mask1*1
        sin_t = torch.sin(t*theta)
        sin = torch.sin(theta) + mask1*1
        q = ( sin_1_t* q1 + sin_t * q2) / sin
        return q
